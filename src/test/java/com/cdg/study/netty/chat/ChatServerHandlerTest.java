...
public class ChatServerHandlerTest {
	...
	public void PING() throws Exception {
		...
		assertThat("PING에 대한 응답은 PONG이 옵니다", m, equalTo(new ChatMessage("PONG")));
	}

	@Test
	public void QUIT() throws Exception {
		m = writeAndRead(new ChatMessage("QUIT"));
		assertThat("QUIT에 대한 응답은 BYE:{nickname} 입니다", m, equalTo(new ChatMessage("BYE", nickname)));
	...
	public void SEND() throws Exception {
		...
		assertThat("SEND를 받으면 FROM을 응답합니다", m.command, equalTo("FROM"));
		assertThat("FROM에는 누가 보낸 메시지인지 nickname이 덧붙습니다", m.nickname, equalTo(nickname));
	...
	public void NICK() throws Exception {
		...
		assertThat("NICK을 받으면 NICK:{old} new를 응답합니다", m, equalTo(new ChatMessage("NICK", nickname, "새닉네임")));
	}

	@Test
	public void UNKNOWN() throws Exception {
		m = writeAndRead(new ChatMessage("UNKNOWN"));
		assertThat("모르는 명령어에는 ERR를 응답합니다", m.command, equalTo("ERR"));
	}
}
